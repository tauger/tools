...
static int trace_event(struct pt_regs *ctx, struct sock *sk, int type)
{
    ...
    if (family == AF_INET) {
        ...
        data4.dport = dport;
        data4.state = state;
        bpf_get_current_comm(&data4.task, 16);
        ...
        data6.dport = dport;
        data6.state = state;
        bpf_get_current_comm(&data6.task, 16);
...
int trace_active_reset(struct pt_regs *ctx, struct sock *sk)
{
    trace_event(ctx, sk, ACTIVE_RESET);
    return 0;
}
"""
...
b.attach_kprobe(event="tcp_send_active_reset", fn_name="trace_active_reset")

# header
print("%-8s %-6s %-2s %-20s %1s> %-20s %-4s %s" % ("TIME", "PID", "IP",
    "LADDR:LPORT", "T", "RADDR:RPORT", "STATE", "PROCESS"))
